...
private:
  ...
  void laserCallback(const sensor_msgs::msg::LaserScan & msg)
  {
    ...
    for (size_t i = 0; i < msg.ranges.size(); i++){
        double current_reading_angle = turtlelib::normalize_angle(static_cast<double>(i) * msg.angle_increment) + msg.angle_min;
    ...
    // now publish markers at the center mean of each cluster
    ...
    for (size_t i = 0; i < clusters.size(); i++){ // for each cluster
        RCLCPP_INFO_STREAM(get_logger(), "cluster "<<i<<" size "<< clusters.at(i).size() <<std::endl);
        
        double mean_x = 0.0;
        double mean_y = 0.0;
        double count = 0.0;
        for (size_t j = 0; j < clusters.at(i).size(); j++){
            mean_x += clusters.at(i).at(j).x;
            mean_y += clusters.at(i).at(j).y;
            count += 1.0;
        }
        mean_x /= count;
        mean_y /= count;
        ...
        // create marker
        auto marker = create_obstacle(0.1, 0.1, 0.2, mean_x, mean_y, 0.1, i, visualization_msgs::msg::Marker::ADD, frame_id);
